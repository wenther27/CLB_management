...
namespace ClubManagement.API.Service
{
    public interface IMemberService
    {
        ...
        Task<MemberDTO?> UpdateAvatarAsync(int userId, UpdateAvatarDTO dto);   // ← MỚI
    ...
    public class MemberService : IMemberService
    {
        ...
        private static MemberDTO MapToDTO(Member m) => new()
        {
            ...
            // Ưu tiên AvatarUrl từ bảng Member (đã crop), fallback User.AvatarUrl nếu có
        ...
        // ── UpdateProfile (thông tin cơ bản) ──────────────────────────────────
        ...
        // ── UpdateAvatar ───────────────────────────────────────────────────────
        /// <summary>
        /// Cập nhật AvatarUrl (thumbnail đã crop) và OriginalAvatarUrl (ảnh gốc).
        /// Được gọi từ PUT /members/me/avatar
        /// </summary>
        public async Task<MemberDTO?> UpdateAvatarAsync(int userId, UpdateAvatarDTO dto)
        {
            ...
            // Luôn cập nhật thumbnail
            member.AvatarUrl = dto.AvatarUrl;

            // Chỉ cập nhật ảnh gốc nếu client gửi lên
            // (khi chọn ảnh mới sẽ gửi, khi chỉ re-crop sẽ không gửi)
            if (dto.OriginalAvatarUrl != null)
                member.OriginalAvatarUrl = dto.OriginalAvatarUrl == string.Empty
                    ? null
                    : dto.OriginalAvatarUrl;
        ...
        // ── Deactivate / ChangePassword / Stats ───────────────────────────────
