...
namespace ClubManagement.API.Service
{
    ...
    public class FundContributionService : IFundContributionService
    {
        ...
        {
            if (await _context.FundCollectionPeriods.AnyAsync(p => p.Year == dto.Year && p.Month == dto.Month))
                return null;
            ...
            var period = new FundCollectionPeriod
            {
                ...
                Contributions = members.Select(m => new FundContribution
                {
                    MemberID = m.MemberID,
                    ExpectedAmount = dto.Amount,
                    PaymentCode = BuildPaymentCode(dto.Year, dto.Month, m.MemberID),
                    Status = "Pending"
                }).ToList()
            };
        ...
        public async Task<FundContributionDTO?> GetMyCurrentContributionAsync(int userId)
        {
            ...
            var contribution = period.Contributions.FirstOrDefault(c => c.MemberID == member.MemberID);
            if (contribution == null)
            {
                contribution = new FundContribution
                {
                    ...
                    PaymentCode = BuildPaymentCode(period.Year, period.Month, member.MemberID),
                    Status = "Pending"
                };
                _context.FundContributions.Add(contribution);
                await _context.SaveChangesAsync();
            }

            return MapContribution(contribution, period);
        }
        ...
        public async Task<(bool Success, string Message)> ProcessSepayWebhookAsync(SepayWebhookDTO dto)
        {
            ...
            var fundTransaction = new FundTransaction
            {
                ...
                Description = $"Đóng quỹ tháng {contribution.Period!.Month:D2}/{contribution.Period.Year} - {contribution.Member!.FullName}",
        ...
        private FundCollectionPeriodDTO MapPeriod(FundCollectionPeriod p)
        {
            ...
            return new FundCollectionPeriodDTO
            {
                ...
                RemainingAmount = (p.Contributions.Count - paid.Count) * p.Amount
            };
        }
        ...
        private static string BuildPaymentCode(int year, int month, int memberId)
            => $"Q{year % 100:D2}{month:D2}M{memberId:D6}";
